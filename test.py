...
def is_artwork(category):
    ...
    result = false
    i = 0
    super_categories = page.Category(commons, category).categories_list()
    while (not result) and  i < len(super_categories):
        result = super_categories[i].title() in finals
    return result
...
def main():
    ...
    with open("categories.json") as data:
        categories = json.loads(data.read().decode())
    loaded_categories = []
    print ("Commons loading")
    for category in categories_list:
        print category
        loaded_categories = loaded_categories + deep_loading(category)
